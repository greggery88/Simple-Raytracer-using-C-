...
inline void render_black_hole(int pixel_width, int samples, int layers, double spacing, char* filename) {
    hittable_list world;

    auto material_sp = make_shared<spacetime>(1);
    auto material_center = make_shared<lambertian>(color(1, 1, 1));

    for (int n = 0; n < layers; n++) {
        world.add(make_shared<sphere>(point3(0, 0, -1), n*spacing, material_sp));
    }
    camera cam;
